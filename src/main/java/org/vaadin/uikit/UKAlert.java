...
public class UKAlert extends Composite<Div> implements HasSize {
	...
	public enum AlertVariant {
		SUCCESS("uk-alert-success"), PRIMARY("uk-alert-primary"), WARNING("uk-alert-warning"),
	...
	public UKAlert(String alertText, AlertVariant variant, int duration, boolean animation) {
		if (variant != null) alert.addClassName(variant.getVariantName());
		p.setText(alertText);
	...
	public void close() {
		alert.getElement().executeJs("UIkit.alert($0).close()", alert.getElement());
	}

    public static class AlertHiddenEvent extends ComponentEvent<UKAlert> {

        public AlertHiddenEvent(UKAlert source,
                boolean fromClient) {
