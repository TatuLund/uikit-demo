...
public class UKGrid extends Composite<Div> implements HasSize, UKWidthAndHeight, UKMargin, UKPadding {
	...
	public class UKGridRow implements Serializable {
		...
		public UKGridRow withCell(int width) {
			...
			if (totalWidth > maxRowWidth) {
				throw new IllegalStateException("Sum of widths can't be larger that maximum row width");
			}
	...
	public void populate(int row, int cell, Component...components) {
		...
		if (row > numRows && cell > rows.get(row).getCellCount()) {
			throw new IllegalStateException("Cell ("+row+","+cell+") does not exists in grid");			
		}
	...
	public void populate(UKGridRow row, int cell, Component...components) {
		...
		if (cell > row.getCellCount()) {
			throw new IllegalStateException("Cell "+cell+" does not exists in this grid row");			
		}
