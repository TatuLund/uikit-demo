...
public class UKRadioGroup<T> extends SelectBase<UKRadioGroup<T>, T>
		implements HasItemsAndComponents<T>, SingleSelect<UKRadioGroup<T>, T>, Focusable<UKRadioGroup<T>>,
		HasDataProvider<T>, UKValidation, UKTooltip, UKFormSizing, UKMargin, UKPadding {
	...
	private static <T> String modelToPresentation(UKRadioGroup<T> select, T model) {
	...
	public UKRadioGroup() {
		super(null, null, String.class, UKRadioGroup::presentationToModel, UKRadioGroup::modelToPresentation);
	}
	...
	private Stream<Radio<T>> getRadios() {
		return getChildren().filter(Radio.class::isInstance).map(child -> (Radio<T>) child);
	}
	...
	private void updateEnabled(Radio<T> radio) {
		boolean disabled = isDisabledBoolean() || !getItemEnabledProvider().test(radio.getItem());
		...
		if (rawValue instanceof Boolean) {
			...
			radio.getElement().setProperty("disabled", disabled ? Boolean.TRUE.toString() : null);
		} else {
	...
	public void setValue(T value) {
		if (value != null) {
			...
			getRadios().forEach(rb -> rb.setChecked(Objects.equals(rb.getItem(), value)));
		}
	}
	...
	private void setupDataProviderListener(DataProvider<T, ?> dataProvider) {
		...
		dataProviderListenerRegistration = dataProvider.addDataProviderListener(event -> {
			if (event instanceof DataChangeEvent.DataRefreshEvent) {
				resetRadio(((DataChangeEvent.DataRefreshEvent<T>) event).getItem());
			} else {
				reset();
			}
		});
	...
	private void reset() {
		...
		getDataProvider().fetch(new Query<>()).map(this::createRadio).forEach(this::add);
	}
