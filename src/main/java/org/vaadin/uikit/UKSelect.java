...
public class UKSelect<T> extends SelectBase<UKSelect<T>, T>
		implements HasItemsAndComponents<T>, SingleSelect<UKSelect<T>, T>, Focusable<UKSelect<T>>, HasDataProvider<T>,
	...
	private static <T> T presentationToModel(UKSelect<T> select, String presentation) {
	...
	public UKSelect() {
		super(null, null, String.class, UKSelect::presentationToModel, UKSelect::modelToPresentation);
...
//                            ((DataChangeEvent.DataRefreshEvent<T>) event).getItem());
	...
	private void reset() {
		...
		getDataProvider().fetch(new Query<>()).map(this::createOption).forEach(this::add);
	}
